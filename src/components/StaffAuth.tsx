  ...
  return (
    <div className="min-h-screen bg-[#0b1118] text-[#e2e8f0] flex items-center justify-center px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-3xl border border-[#223040] bg-[#0f1722] p-6 shadow-2xl">
        ...
        <p className="text-xs text-[#cbd5e1] mb-4">Sign in with your staff email and password.</p>
        <div className="flex items-center gap-2 rounded-2xl border border-[#223040] bg-[#0b1118] px-3 py-3 mb-3">
          <Mail className="w-5 h-5 text-emerald-400" />
          <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="w-full bg-transparent text-[#e2e8f0] placeholder:text-[#cbd5e1] focus:outline-none" />
        </div>
        <div className="flex items-center gap-2 rounded-2xl border border-[#223040] bg-[#0b1118] px-3 py-3 mb-4">
          <Lock className="w-5 h-5 text-emerald-400" />
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full bg-transparent text-[#e2e8f0] placeholder:text-[#cbd5e1] focus:outline-none" />
        </div>
...
export const StaffHeader: React.FC<{ staff: any; onLogout: () => void }> = ({ staff, onLogout }) => {
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b border-[#223040] bg-[#0f1722]/60">
      <div>
        <div className="text-xs text-[#cbd5e1] font-semibold">{staff.email}</div>
        <div className="text-[11px] text-[#cbd5e1]">{staff.role === 'superadmin' ? 'Global admin' : `Home admin: ${staff.homeId}`}</div>
      </div>
