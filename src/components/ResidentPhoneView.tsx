...
export const ResidentPhoneView: React.FC<ResidentPhoneViewProps> = ({
  ...
  return (
    <div className="py-6 px-4 max-w-5xl mx-auto">
      ...
      <div className="mb-6 bg-[#0f1722] rounded-2xl p-4 sm:p-5 border border-[#1e293b] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition flex items-center gap-1 ${
              isLargeText 
                ...
                : 'bg-[#0f1722] text-[#e2e8f0] border-[#1e293b] hover:bg-[#141d27]'
            }`}
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition flex items-center gap-1 ${
              isHighContrast 
                ? 'bg-[#0f1722] text-[#e2e8f0] border-[#223040]' 
                : 'bg-[#0f1722] text-[#e2e8f0] border-[#1e293b] hover:bg-[#141d27]'
            }`}
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-[#0f1722] hover:bg-[#131d27] text-[#e2e8f0] transition flex items-center gap-1 shadow-sm"
          >
      ...
      <div className={`flex justify-center transition-all duration-300 ${
        isFullScreen 
          ? 'fixed inset-0 z-50 bg-[#0b1118]/90 backdrop-blur-md p-2 sm:p-6 overflow-y-auto' 
          : 'relative'
      }`}>
        <div className={`w-full max-w-[420px] rounded-3xl sm:rounded-[44px] p-2 sm:p-3 transition-all duration-300 ${
          isHighContrast ? 'bg-black border-2 sm:border-4 border-[#223040]' : 'bg-[#0f1722] border-4 sm:border-[10px] border-[#223040] shadow-2xl'
          ...
          {isFullScreen && (
            <button
              ...
              className="absolute -top-3 -right-3 z-50 p-2.5 rounded-full bg-[#131d27] text-[#e2e8f0] hover:bg-[#131d27] shadow-xl border border-[#223040]"
          ...
          <div className="w-full flex justify-center pt-2 pb-3">
            <div className="w-24 h-4 rounded-full bg-[#0b1118] flex items-center justify-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-[#131d27]"></div>
              <div className="w-10 h-1.5 rounded-full bg-[#131d27]"></div>
            </div>
          </div>

          {/* Inner Phone Screen */}
          <div className={`flex-1 rounded-[32px] overflow-hidden flex flex-col relative ${
            isHighContrast ? 'bg-[#0b1118] text-[#e2e8f0]' : 'bg-[#0f1722] text-[#e2e8f0]'
          }`}>
            ...
            {showUndoNotice && (
              <div className="bg-amber-500/100/90 text-[#e2e8f0] px-4 py-2 text-xs font-bold flex items-center justify-between shadow-md animate-in slide-in-from-top duration-200">
                <span>Response recorded! Tapped by mistake?</span>
                <button
                  id="undo-tap-btn"
                  onClick={handleUndo}
                  className="px-2.5 py-1 rounded bg-[#0b1118] text-[#e2e8f0] hover:bg-[#131d27] transition flex items-center gap-1"
                >
            ...
            <div className="flex-1 p-4 flex flex-col justify-center gap-4">
              {resident.status === 'awaiting' ? (
                ...
                <div className="flex-1 flex flex-col gap-4 justify-center">
                  ...
                  <button
                    ...
                    className={`flex-1 w-full rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all duration-150 active:scale-95 shadow-xl border-4 ${
                      isHighContrast
                        ? 'bg-emerald-500/200 hover:bg-emerald-400 text-[#e2e8f0] border-white'
                        : 'bg-gradient-to-b from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-emerald-950 border-emerald-500/40/40'
                    }`}
                    style={{ minHeight: '190px' }}
                  >
                    <div className="w-20 h-20 rounded-full bg-emerald-950/20 flex items-center justify-center mb-3 shadow-inner">
                      <Check className="w-12 h-12 text-[#e2e8f0] stroke-[3.5]" />
                    </div>
                    <span className={`font-black uppercase tracking-tight text-[#e2e8f0] drop-shadow-sm ${
                      isLargeText ? 'text-3xl' : 'text-2xl'
                    }`}>
                      I&apos;M OKAY
                    </span>
                  ...
                  <button
                    ...
                    className={`flex-1 w-full rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all duration-150 active:scale-95 shadow-xl border-4 ${
                      isHighContrast
                        ? 'bg-rose-600 hover:bg-rose-500/150 text-[#e2e8f0] border-white'
                        : 'bg-gradient-to-b from-rose-600 to-rose-700 hover:from-rose-500 hover:to-rose-600 text-[#e2e8f0] border-rose-400/40'
                    }`}
                    style={{ minHeight: '190px' }}
                  >
                    <div className="w-20 h-20 rounded-full bg-rose-950/30 flex items-center justify-center mb-3 shadow-inner">
                      <LifeBuoy className="w-12 h-12 text-[#e2e8f0] stroke-[3]" />
                    </div>
                    <span className={`font-black uppercase tracking-tight text-[#e2e8f0] drop-shadow-sm ${
                      isLargeText ? 'text-3xl' : 'text-2xl'
                    }`}>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-5 bg-gradient-to-b from-emerald-950/40 to-slate-900 rounded-3xl border border-emerald-500/30 animate-in fade-in zoom-in duration-300">
                  <div className="w-24 h-24 rounded-full bg-emerald-500/200 flex items-center justify-center mb-4 shadow-lg shadow-emerald-500/30">
                    <CheckCircle2 className="w-14 h-14 text-[#e2e8f0]" />
                  </div>
                  
                  <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">
                    Check-in Completed
                  </span>
                  <h3 className={`font-black text-[#e2e8f0] mt-1 ${
                    isLargeText ? 'text-3xl' : 'text-2xl'
                  }`}>
                    You&apos;re Checked In!
                  </h3>
                  
                  <p className="text-[#e2e8f0] text-sm mt-2 max-w-xs leading-relaxed">
                    Have a wonderful morning, <strong className="text-[#e2e8f0]">{resident.name}</strong>. Care staff has been notified.
                  </p>

                  <div className="mt-4 px-4 py-2 rounded-xl bg-[#131d27]/80 border border-[#223040]/60 text-[#e2e8f0] text-xs font-mono">
                  ...
                  <div className="mt-6 pt-6 border-t border-[#223040] w-full flex flex-col gap-2">
                    ...
                    <button
                      ...
                      className="w-full py-3 rounded-xl bg-[#131d27] hover:bg-[#131d27] text-[#e2e8f0] text-xs font-bold transition flex items-center justify-center gap-2"
                    >
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-5 bg-gradient-to-b from-rose-950/60 to-slate-900 rounded-3xl border-2 border-rose-500/60 animate-in fade-in zoom-in duration-300">
                  <div className="w-24 h-24 rounded-full bg-rose-600 flex items-center justify-center mb-4 shadow-lg shadow-rose-600/40 animate-pulse">
                    <AlertTriangle className="w-14 h-14 text-[#e2e8f0]" />
                  </div>
                  ...
                  <p className="text-[#e2e8f0] text-sm mt-2 max-w-xs leading-relaxed font-medium">
                    Care staff has been notified for <strong className="text-[#e2e8f0]">{resident.room}</strong>. Please rest comfortably.
                  </p>

                  {/* Immediate Emergency Calling Action */}
                  <a
                    href="tel:0825550000"
                    className="mt-5 w-full py-4 rounded-2xl bg-rose-600 hover:bg-rose-500/150 text-[#e2e8f0] font-black text-base flex items-center justify-center gap-3 shadow-lg shadow-rose-900/50 active:scale-95 transition"
                  >
          ...
          <div className="w-full flex justify-center py-2">
            <div className="w-32 h-1 rounded-full bg-[#131d27]"></div>
          </div>
        </div>
      </div>
      ...
      {/* 1-Tap Pairing Modal */}
      {showPairModal && (
        <div className="fixed inset-0 z-50 bg-[#0f1722]/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0f1722] rounded-3xl p-6 max-w-md w-full shadow-2xl border border-[#1e293b] text-[#e2e8f0]">
            ...
            <div className="py-5 text-center flex flex-col items-center">
              {/* Simulated QR Code */}
              <div className="w-48 h-48 rounded-2xl bg-[#141d27] border-2 border-dashed border-[#223040] p-3 flex flex-col items-center justify-center shadow-inner relative">
                    ...
                    <div 
                      ...
                      className={`rounded-xs ${
                        (i % 2 === 0 && i % 3 === 0) || i === 0 || i === 5 || i === 30 || i === 35 
                          ? 'bg-[#0f1722]' 
                          : (i % 5 === 0 ? 'bg-emerald-600' : 'bg-[#141d27]')
                      }`} 
                    />
                  ))}
                </div>
              ...
              <div className="mt-3 w-full bg-[#141d27] rounded-xl p-2.5 text-left font-mono text-[11px] text-[#e2e8f0] break-all border border-[#1e293b]">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-[#e2e8f0] font-bold text-xs transition"
              >
                Copy SMS Safe Link
              </button>
              <button
                onClick={() => setShowPairModal(false)}
                className="px-4 py-2.5 rounded-xl bg-[#141d27] hover:bg-[#141d27] text-[#e2e8f0] font-bold text-xs transition"
              >
                Done
              </button>
            </div>
